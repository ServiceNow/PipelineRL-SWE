...
TIMESTAMP=$(date +%s)
RUN_NAME=${RUN_NAME:-gpt_oss_reasoning_smoke_single_node}
OUTPUT_ROOT=${OUTPUT_ROOT:-/mnt/llmd/results/exps/aristides/reason/${RUN_NAME}_${TIMESTAMP}}
...
echo "Starting gpt-oss server on ${HOST}:${PORT} using CUDA_VISIBLE_DEVICES=${CUDA_DEVICES}"
(
