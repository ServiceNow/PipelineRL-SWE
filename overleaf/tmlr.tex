...
SWE-bench Verified.  The value lies in knowing which problems to attempt: on LiveCodeBench a give-up action pays only
with per-query beliefs, and the same probe does not predict which model suits a problem, capturing
none of 13.5 points of routing headroom on a five-lab pool.  The signal transfers to API-only models
...
\item \textbf{Whether, not which} (Section~\ref{sec:whethernotwhich}).  The probe reads a
difficulty shared across the pool, not which model suits a problem.  On a five-lab pool with
13.5 points between the best model and the pool's oracle, a router on the probe uses one model
for 171 of 171 test problems.
...
the decision whether to attempt a problem and not the choice of model, and the difference is
visible in measurable pool structure (Section~\ref{sec:whethernotwhich}).  We claim neither the
...
\subsection{Whether, not which}
\label{sec:whethernotwhich}

The probe predicts whether a problem is worth attempting.  It does not predict which model to
use.
...
problems --- the router collapse described by \citet{routingcollapse2026}.  On our own
three-route pool, single-commit routing by the probe captures 22--31\% of the oracle's gain
(Appendix~\ref{app:singleshot}), because one model dominates.  A one-dimensional
latent can say how hard a problem is, which is a threshold and therefore an abstention decision;
it cannot say which model suits the problem, which needs the models' ordering to change from
problem to problem.  Adding dimensions lowers accuracy per dollar.
...
better than two-step, and the exact solve is worse at the 84\% target.  The lattice treats a
failure on one route as uninformative about the others, which over-values continuation; the
...
the same policy gains little or nothing from its give-up action.  The beliefs say how hard a problem is,
not which model suits it, so their value is in deciding whether to attempt a problem.  They
