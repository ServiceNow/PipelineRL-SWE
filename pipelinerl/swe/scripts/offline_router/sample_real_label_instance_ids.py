...
def _sample(ids: list[str], n: int, seed: int) -> list[str]:
    if n <= 0:
        raise ValueError("sample size must be positive")
    if n > len(ids):
        logger.warning("Requested %s ids from only %s available; using all", n, len(ids))
        n = len(ids)
...
def main() -> None:
    ...
    train_ids = _sample(train_ids_all, args.train_n, args.seed)
    # Offset eval seed so identical identifiers cannot be selected by identical RNG state if splits overlap.
    eval_ids = _sample(eval_ids_all, args.eval_n, args.seed + 1009)
