#!/usr/bin/env bash
# The head-to-head that has never been run: LEARNED sigma against OUR PROBE, on the 64k pool.
#
# PAPER_OUTLINE 3b-xxxi: "that run carries no `content_decay` arm, so the factorized belief has
# never been compared head-to-head against our probe, and it has never been re-run on the 64k
# pool. The open question is whether it beats ours, not whether it works."
#
# Two changes from launch_lcb_mdp_factorized.sh, both required for the comparison to exist:
#   * tensors point at lcb_pool64k_full_prepared, the pool every current number is measured on;
#   * --content-preds / --cost-preds are passed, so `content_decay*` arms are built in the SAME
#     replay as `sequential*`. Comparing across replays would confound the seed, the draw
#     orderings and the budget grid with the belief source.
# The budget grid is geometric to match the value arm's spacing law (3b-xxxv).
#
...
SEED=${SEED:-17}
SOURCE_ARTIFACT=${SOURCE_ARTIFACT:-/mnt/llmd/results/exps/aristides/reason/lcb_pool64k_full_prepared}
TENSORS_DIR=${TENSORS_DIR:-${SOURCE_ARTIFACT}/tensors_v3}
JOB_NAME=${JOB_NAME:-lcb_fact64k_h2h_seed${SEED}_${TIMESTAMP}}
PREP=${PREP:-/mnt/llmd/results/exps/aristides/reason/lcb_pool64k_full_prepared}
...
Prepared but not submitted.
  ...
  output: ${OUTPUT_DIR}
  compare against: content_decay* arms IN THE SAME replay (that is the point)
...
python pipelinerl/swe/scripts/livecodebench/replay_mdp_full_execution.py \
  ...
  --content-preds ${PREP}/content_preds_rich_cal.jsonl \
  --cost-preds ${PREP}/cost_preds_rich_ts.jsonl \
  --probe-cost-usd 0.000149 --pseudo-count 0.71 --decay-pseudo-count 0.95 \
  --value-grid-points 96 --budget-grid geometric --budget-grid-points 96 \
